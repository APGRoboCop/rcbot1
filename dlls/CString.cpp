...
CString& CString::TakeOwnership( char* pszString )
{
	...
	{
		ALERT( at_error, "CString::TakeOwnership: null pointer passed; can't take ownership!\n" );
...
void CString::VFormat( const char* pszFormat, va_list list )
{
	...
	if( iLength < 0 || iLength >= sizeof( szBuffer ) )
	{
		...
		ALERT( at_error, "Error formatting string: format used: '%s'\n", pszFormat );
		return;
	}
